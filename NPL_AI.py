...
def calcular_similitud(texto, pregunta):
    ...
    similitud = cosine_distances(matriz[0:1], matriz[1:2])  # Ajuste en los índices
    return similitud[0][0]

# Función para realizar la pregunta a la API de OpenAI (usando la nueva API)
def realizar_pregunta_a_openai(pregunta, contenido_txt):
    response = openai.ChatCompletion.create(
        model="gpt-4o",  # Puedes usar "gpt-4" si tienes acceso
        messages=[
            {"role": "system", "content": "Eres un asistente experto en NLP."},
            {"role": "user", "content": f"Archivo de texto:\n\n{contenido_txt}\n\nPregunta: {pregunta}\n"}
        ]
    )
